...
import org.mybatis.generator.plugins.ModelColumnPlugin;
...
//            importTypes.addAll(method.getReturnType().getTypeArguments());
            importTypes.addAll(method.getExceptions());
        }
...
//                }
//            }
            for (Element ele : rootElement.getElementsOld()) {
                if (ele instanceof XmlElement && ((XmlElement) ele).getName().equals("sql")) {
                    index++;
                }
            }
...
//                List<Element> elements = rootElement.getElements();
                List<Element> elements = rootElement.getElementsOld();
                int index = -1;
...
//        Iterator<Element> elementIterator = element.getElements().iterator();
        Iterator<Element> elementIterator = element.getElementsOld().iterator();
        boolean flag = false;
...
//            element.addElement(0, tmpEle.getElements().get(i));
            element.addElement(0, tmpEle.getElementsOld().get(i));
        }
    }
